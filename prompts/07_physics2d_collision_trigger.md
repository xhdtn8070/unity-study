# 07_physics2d_collision_trigger.md
...
현재 단계: Chapter 7 - 2D Physics / Collision / Trigger

이번 단계의 목표:
- 물리 충돌과 상호작용 감지를 구분하고 Rigidbody2D / Collider2D / Trigger를 역할별로 이해하게 만든다.

반드시 다뤄야 할 세부 포인트:
- Rigidbody2D와 Collider2D 조합
- Is Trigger
- Physics 2D Layer Collision Matrix 개념
- 벽 충돌과 상호작용 범위의 차이
- Trigger 디버깅 감각

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- 플레이어가 벽에 막히고, 상자 근처에서는 Trigger로 상호작용 문구가 뜨게 한다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- 충돌과 상호작용을 하나의 콜라이더로 다 해결하려는 실수
- 레이어 충돌 설정을 안 보고 코드만 고치는 실수
- Rigidbody2D 설정을 이해하지 않고 값만 만지는 실수
