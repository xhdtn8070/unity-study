...
현재 단계: Chapter 3 - Inspector / Serialization

이번 단계의 목표:
- Unity 직렬화와 Inspector 노출 규칙을 이해해서 튜닝 가능한 값을 에디터에서 다룰 수 있게 만든다.

반드시 다뤄야 할 세부 포인트:
- public field와 [SerializeField] private field
- property가 기본 직렬화 대상이 아닌 이유
- Serializable class
- OnValidate의 위치와 용도(맛보기 수준)
- Inspector 기반 튜닝 감각

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- PlayerConfig 컴포넌트를 만들고 속성 4개를 다양한 방식으로 선언해 Inspector 표시 차이를 본다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- 일반 C# 캡슐화 습관만으로 Inspector 친화성이 깨지는 실수
- Dictionary를 바로 직렬화하려는 실수
- 런타임 값과 튜닝 값을 구분하지 않는 실수
