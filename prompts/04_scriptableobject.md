...
현재 단계: Chapter 4 - ScriptableObject

이번 단계의 목표:
- 설정/룰/데이터 자산과 씬/프리팹/런타임 상태를 분리해서 생각하게 만든다.

반드시 다뤄야 할 세부 포인트:
- ScriptableObject asset의 정체
- CreateAssetMenu
- ItemData / EnemyArchetype / DifficultyProfile 예시
- 저작용 데이터 vs 런타임 상태
- 세이브 시스템과 혼동하지 않는 법

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- ItemData ScriptableObject를 만들고 이름/가격/아이콘/설명 asset 3개를 만든다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- SO를 mutable runtime singleton처럼 쓰는 실수
- SO와 MonoBehaviour의 책임을 섞는 실수
- SO를 저장파일처럼 이해하는 실수
