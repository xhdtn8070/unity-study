...
현재 단계: Chapter 9 - Runtime UI

이번 단계의 목표:
- 현재 프로젝트에 맞는 UI 시스템을 선택하고, 최소 HUD를 만들 수 있게 한다.

반드시 다뤄야 할 세부 포인트:
- Unity UI 시스템 3종(UI Toolkit / uGUI / IMGUI) 개요
- 왜 이번 단계에서는 무엇을 기본으로 고를지
- HUD에 필요한 최소 정보
- Canvas 또는 UIDocument의 역할
- 게임 상태를 플레이어에게 보여주는 관점

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- Day/Gold/Stamina/Interact Hint만 보이는 최소 HUD를 만든다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- UI 시스템 선택 기준 없이 튜토리얼을 섞는 실수
- HUD와 메뉴, 디버그 UI를 한 번에 만들려는 실수
- 표시 정보보다 데이터 구조를 먼저 거대하게 만드는 실수
