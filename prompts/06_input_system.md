...
현재 단계: Chapter 6 - Input System

이번 단계의 목표:
- 키 입력을 하드코딩하지 않고 행동(Action) 중심으로 모델링하게 만든다.

반드시 다뤄야 할 세부 포인트:
- Input Actions asset
- Move / Interact / UseTool 같은 행동 단위
- PlayerInput 또는 코드 바인딩 맛보기
- Active Input Handling 점검 포인트
- 입력 디버깅 기본

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- Move와 Interact 액션만 가진 Input Actions asset을 만들고, 키 입력시 디버그 로그를 출력한다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- Update에 키 체크를 흩뿌리는 실수
- 입력과 게임 규칙을 강하게 결합하는 실수
- 키보드 기준으로만 설계하는 실수
