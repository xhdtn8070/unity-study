# 08_animator_state_transition.md
...
현재 단계: Chapter 8 - Animator / State / Transition

이번 단계의 목표:
- 애니메이션을 파일 재생이 아니라 상태와 전이 조건으로 생각하게 만든다.

반드시 다뤄야 할 세부 포인트:
- Animator Controller
- State, Transition, Parameter
- Idle / Walk / UseTool 최소 구조
- 속도값/불린값과 상태 전이의 연결
- 애니메이션 디버깅 관찰 포인트

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- Idle과 Walk 두 상태만으로 플레이어 이동 애니메이션을 전이시킨다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- 애니메이션 클립과 상태 머신을 구분 못하는 실수
- 모든 상태를 코드 if문으로 처리하려는 실수
- 전이 조건을 과하게 복잡하게 만드는 실수
