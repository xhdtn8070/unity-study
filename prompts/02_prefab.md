...
현재 단계: Chapter 2 - Prefab

이번 단계의 목표:
- 반복 오브젝트를 씬 복붙이 아니라 Prefab asset / instance / override로 이해하게 만든다.

반드시 다뤄야 할 세부 포인트:
- Prefab asset vs scene instance
- Overrides, Apply, Revert
- Prefab Variant
- Project 창에서 asset로 보는 관점
- 씬 오브젝트를 자산화하는 흐름

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- Crate 오브젝트를 만들고 Prefab으로 저장한 뒤, 씬 인스턴스에서 값을 바꾸고 Override를 관찰한다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- 씬에서 복붙으로 반복 오브젝트를 늘리는 실수
- Prefab과 instance 수정 차이를 모르는 실수
- 모든 것을 상속 구조로 해결하려는 실수
