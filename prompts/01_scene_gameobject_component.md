# 01_scene_gameobject_component.md
...
현재 단계: Chapter 1 - Scene / GameObject / Component

이번 단계의 목표:
- Unity를 '공간(Scene) + 존재(GameObject) + 기능(Component)'로 분해해서 생각하게 만든다.

반드시 다뤄야 할 세부 포인트:
- Scene과 Project asset의 차이
- Hierarchy / Scene / Game / Inspector / Project 창의 역할
- GameObject 생성과 Transform
- Component 추가와 제거
- Main Camera와 SpriteRenderer를 눈으로 확인하는 법

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- 빈 씬 하나에서 Player, Ground, Main Camera를 만들고, 각 오브젝트에 어떤 Component가 붙는지 직접 확인한다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- Scene을 코드 모듈처럼 생각하는 실수
- GameObject와 Component를 클래스로만 이해하는 실수
- Inspector를 '디버그 창' 정도로만 보는 실수
