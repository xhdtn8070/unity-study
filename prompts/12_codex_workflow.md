...
현재 단계: Chapter 12 - Codex Workflow

이번 단계의 목표:
- AGENTS.md, skills, MCP, 계획-구현-리뷰 루프를 스스로 운영하게 만든다.

반드시 다뤄야 할 세부 포인트:
- AGENTS.md의 역할
- skills와 재사용 워크플로
- MCP로 실제 프로젝트 읽기
- 좋은 프롬프트 계약(Goal/Context/Constraints/Done)
- 학습 세션과 리뷰 세션 분리

이번 단계의 10분 실습은 반드시 이것을 중심으로 설계하라:
- 하나의 작은 작업을 Plan → Implement → Review 순서로 운영해 본다.

이번 단계에서 특히 경계해야 할 흔한 실수:
- 매번 새 프롬프트로 룰을 초기화하는 실수
- MCP 없이 추측으로 큰 리팩터링을 하는 실수
- 프롬프트에 완료 조건을 안 적는 실수
